...
public:
	...
	*/
	...
	protected:
		Node<Key, Value>* mCurrent; // this is a node to the current node that is being analyzed by the iterator
	};

public:
	iterator begin(); // returns an iterator to the beginning of the tree 
	iterator end(); // returns an iterator to the end of the tree 
...
void BinarySearchTree<Key, Value>::insert(const std::pair<Key, Value>& keyValuePair)
{
	...
	bool exit = false;
	while(!exit){
		if(keyValuePair.second < currentNode->getKey()){
		...
		}else if(keyValuePair.second > currentNode->getKey()){
...
/*
 * This is a helper for the clear function that uses postorder traversal to clear the tree.
*/
template<typename Key, typename Value>
void BinarySearchTree<Key, Value>::clearHelper(Node<Key,Value>*currentNode){ 
	if(currentNode == nullptr){
		return;
	}
	this->clearHelper(currentNode->getLeft());
	this->clearHelper(currentNode->getRight());
	delete currentNode;
}
...
Node<Key, Value>* BinarySearchTree<Key, Value>::getSmallestNode() const
{
	...
	return this->internalFind(it->begin());
}
